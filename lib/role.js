...
// delete roles
const deleteRoles = () => {
  let sql = `SELECT * dpt.id, title FROM roles`;

  db.query(sql, (error, response) => {
    if (error) throw error;
    let rolesNames = [];
    response.forEach((roles) => {
      rolesNames.push(roles.title);
    });
...
module.exports = { viewAllRolesMenu, addRoleMenu, deleteRoles, Role };
